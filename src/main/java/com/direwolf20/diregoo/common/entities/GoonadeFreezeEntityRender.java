...
public class GoonadeFreezeEntityRender extends EntityRenderer<GoonadeFreezeEntity> {

    public GoonadeFreezeEntityRender(EntityRendererManager renderManager) {
        super(renderManager);
        this.shadowSize = 0F;
    }

    @Override
    public void render(GoonadeFreezeEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        BlockRendererDispatcher blockrendererdispatcher = Minecraft.getInstance().getBlockRendererDispatcher();
        IRenderTypeBuffer.Impl buffer = Minecraft.getInstance().getRenderTypeBuffers().getBufferSource();
        IVertexBuilder builder;
        Minecraft mc = Minecraft.getInstance();
        builder = buffer.getBuffer(OurRenderTypes.RenderBlock);
        BlockState renderBlockState = ModBlocks.GOO_BLOCK.get().getDefaultState().with(GooBase.FROZEN, 1);
    ...
    @Override
    public ResourceLocation getEntityTexture(GoonadeFreezeEntity entity) {
        return null;
    }
}